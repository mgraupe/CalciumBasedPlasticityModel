...
sChangeStoch = zeros((len(freq),2))
sChangeJesStoch = zeros((len(freq)))
#synChangeStochOld = zeros((len(freq)))


deltaTs = linspace(-0.05,0.05,1001)
sChange2 = zeros(len(deltaTs))
sChange2Stoch = zeros(len(deltaTs))
...
for i in range(len(freq)):
    ...
    #
    (alphaD, alphaP) = tat.spikePairFrequencySTPStochastic(0.01 - synChange.D, freq[i], synChange.Npairs,
                                                              synChange.tauRec, synChange.U,synChange.Nvesicles)
    synChange.changeInSynapticStrength(synChange.Npresentations, w0, alphaD, alphaP)
    # (alphaD,alphaP) = tat.spikePairFrequencyNonlinear(0.01-synChange.D,freq[i])
    # synChange.changeInSynapticStrength(Npresentations/freq[i],w0,alphaD,alphaP)
    sChangeStoch[i, 0] = synChange.mean / w0
    ...
    # (alphaD,alphaP) = tat.spikePairFrequencyNonlinear(0.01-synChange.D,freq[i])
    # synChange.changeInSynapticStrength(Npresentations/freq[i],w0,alphaD,alphaP)
    sChangeStoch[i, 1] = synChange.mean / w0
    ...
    synChange.changeInSynapticStrength(synChange.Npresentations,w0,alphaD,alphaP)
    sChangeJesStoch[i] = synChange.mean/w0
    print freq[i]
...
for i in range(len(deltaTs)):
    ...
    #################################
    (alphaD, alphaP) = tat.spikePairFrequencySTPStochastic(deltaTs[i] - synChange.D, 0.1, synChange.Npairs,
                                                              synChange.tauRec, synChange.U, synChange.Nvesicles)
    synChange.changeInSynapticStrength(synChange.Npresentations, w0, alphaD, alphaP)
    # (alphaD,alphaP) = tat.spikePairFrequencyNonlinear(deltaTs[i]-synChange.D,0.1)
    # synChange.changeInSynapticStrength(Npresentations/0.1,0.5,alphaD,alphaP)
    sChange2Stoch[i] = synChange.mean / w0
    print deltaTs[i]
...
ax0.plot(freq,sChangeStoch[:,0],lw=2,color='magenta')
ax0.plot(freq,sChangeStoch[:,1],lw=2,color='green')
...
ax4.set_title('Markram : stochastic pairs (50 pairs)')
...
ax4.plot(freq,sChangeJesStoch,lw=2,color='green')
...
ax1.plot(deltaTs*1000.,sChange2Stoch,lw=2,color='blue')
...
irrPairsDTStoch = np.load(dir0+'irregularSpikePairs_vs_deltaT_differentFreqs_STDstoch_%s.npy' % parameterSet)
regPairsDTStoch = np.load(dir0+'regularSpikePairs_vs_deltaT_differentFreqs_STDstoch_%s.npy' % parameterSet)
...
#################################################################################
gssub2 = gridspec.GridSpecFromSubplotSpec(2, 3, subplot_spec=gs[2],hspace=0.35,wspace=0.3)

irrPairsFreq = np.load(dir0+'irregularSpikePairs_vs_rate_differentDeltaTs_STDdet_%s.npy' % parameterSet)
irrPairsFreqStoch = np.load(dir0+'irregularSpikePairs_vs_rate_differentDeltaTs_STDstoch_%s.npy' % parameterSet)


ax12 = plt.subplot(gssub2[0])
ax12.set_title('deterministic STD, irregular pairs vs. rate (10 sec)')
...
ax12 = plt.subplot(gssub2[3])
ax12.set_title('stochastic STD, irregular pairs vs. rate (10 sec)')

ax12.axhline(y=1,ls='--',color='0.6',lw=2)

ax12.plot(irrPairsFreqStoch[:,5],irrPairsFreqStoch[:,16]/w0,lw=2,c='red',label=r'$\Delta t = %s$ ms, $\rho=%s$' % (irrPairsFreqStoch[0,4]*1000.,irrPairsFreqStoch[0,11]))
ax12.plot(irrPairsFreqStoch[:,5],irrPairsFreqStoch[:,15]/w0,lw=2,c='red',alpha=0.6,label=r'$\Delta t = %s$ ms, $\rho=%s$' % (irrPairsFreqStoch[0,3]*1000.,irrPairsFreqStoch[0,10]))
ax12.plot(irrPairsFreqStoch[:,5],irrPairsFreqStoch[:,14]/w0,lw=2,c='k',label=r'$\rho=%s$' % (irrPairsFreqStoch[0,9]))
ax12.plot(irrPairsFreqStoch[:,5],irrPairsFreqStoch[:,13]/w0,lw=2,c='blue',alpha=0.6,label=r'$\Delta t = %s$ ms, $\rho=%s$' % (irrPairsFreqStoch[0,1]*1000.,irrPairsFreqStoch[0,8]))
ax12.plot(irrPairsFreqStoch[:,5],irrPairsFreqStoch[:,12]/w0,lw=2,c='blue',label=r'$\Delta t = %s$ ms, $\rho=%s$' % (irrPairsFreqStoch[0,0]*1000.,irrPairsFreqStoch[0,7]))
#ax12.plot(irrPairsDT[:,3][argM],irrPairsDT[:,16][argM]/w0,'o',c='k')
#ax12.plot(irrPairsDT[:,3][argP],irrPairsDT[:,16][argP]/w0,'o',c='k')
#ax12.plot(irrPairsDT[:,5][argM],irrPairsDT[:,18][argM]/w0,'>',c='k')
#ax12.plot(irrPairsDT[:,5][argP],irrPairsDT[:,18][argP]/w0,'>',c='k')
...
####################
########################################
ax1 = plt.subplot(gssub2[4])


ax1.axhline(y=0,color='0.4',ls='--')

ax1.plot(irrPairsFreqStoch[:,5],(irrPairsFreqStoch[:,16]-irrPairsFreqStoch[:,14])/w0,color='r',lw=3,label=r'$\rho=0.4$, $\Delta = 10$ ms')
ax1.plot(irrPairsFreqStoch[:,5],(irrPairsFreqStoch[:,15]-irrPairsFreqStoch[:,14])/w0,c='red',alpha=0.6,lw=3,label=r'$\rho=0.2$, $\Delta = 10$ ms')

ax1.plot(irrPairsFreqStoch[:,5],(irrPairsFreqStoch[:,12]-irrPairsFreqStoch[:,14])/w0,color='b',lw=3,label=r'$\rho=0.2$, $\Delta = -10$ ms')
ax1.plot(irrPairsFreqStoch[:,5],(irrPairsFreqStoch[:,13]-irrPairsFreqStoch[:,14])/w0,c='blue',alpha=0.6,lw=3,label=r'$\rho=0.4$, $\Delta = -10$ ms')
...
rho04_interp = interp1d(irrPairsFreqStoch[:,5], (irrPairsFreqStoch[:,16] - irrPairsFreqStoch[:,14]) / w0)
rho02_interp = interp1d(irrPairsFreqStoch[:,5], (irrPairsFreqStoch[:,15] - irrPairsFreqStoch[:,14]) / w0)
rho0_interp = interp1d(irrPairsFreqStoch[:,5], irrPairsFreqStoch[:,14] / w0)
