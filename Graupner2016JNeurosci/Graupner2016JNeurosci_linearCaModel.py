...
#
synChange = synapticChange('sjoestroemNoSTD',par,fromFile=True,nonlinear=nonlinear)
...
for i in range(len(freq)):
    ...
    (alphaD,alphaP) = tat.spikePairFrequency(0.01-synChange.D,freq[i])
...
for i in range(len(deltaTs)):
    (alphaD,alphaP) = tat.spikePairFrequency(deltaTs[i]-synChange.D,0.1)
...
ax0.errorbar(synChange.xDataReg[:,0][::2],synChange.yDataReg[::2],yerr=synChange.sigmaDataReg[::2],fmt='s',color='red',clip_on=False)
ax0.errorbar(synChange.xDataReg[:,0][1::2],synChange.yDataReg[1::2],yerr=synChange.sigmaDataReg[1::2],fmt='o',color='blue',clip_on=False)
...
ax4.set_title('Sjoestroem : stochastic (75 pairs)')
...
idx = 2
ax81.set_title('irregular pairs vs. $\Delta t$ (10 sec)'+'\n'+'%s Hz' % irrPairsDT[0,idx])
...
gssub2 = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=gs[2],hspace=0.2,wspace=0.2)
...
ax12.text(70,1.1,ttt % (synChange.mse,synChange.tauCa,synChange.Cpre,synChange.Cpost,synChange.thetaD,synChange.thetaP,synChange.gammaD,synChange.gammaP,synChange.tau,synChange.D))    
